...
def main(config_name='Sod'):
    ...
    flux_limiters = {
        ...
        "Koren": utils.koren,
        "MC": utils.MC,
        "DPFL (linear)": neural_flux_limiter_linear,
    }
    ...
    errors = np.zeros((3, len(flux_limiters), n_datapoints)) # 3 for rho, u, p
    sol_list = []
    for n, n_cells in enumerate(cell_counts):
        ...
        print(f'Computing for N={n_cells}.')
        for i, (name, flux_limiter) in enumerate(flux_limiters.items()):
            ...
            # Use L2 error
            # errors[0, i, n] = np.sqrt(np.mean((rho - rho_a)**2))
            # errors[1, i, n] = np.sqrt(np.mean((u - u_a)**2))
            # errors[2, i, n] = np.sqrt(np.mean((p - p_a)**2))

            # Use L1 error
            errors[0, i, n] = np.mean(np.abs(rho - rho_a))
            errors[1, i, n] = np.mean(np.abs(u - u_a))
            errors[2, i, n] = np.mean(np.abs(p - p_a))
    ...
    orders_0 = np.vstack([conv_order(errors[0, i]) for i in range(errors.shape[1])])
    orders_1 = np.vstack([conv_order(errors[1, i]) for i in range(errors.shape[1])])
    orders_2 = np.vstack([conv_order(errors[2, i]) for i in range(errors.shape[1])])
    orders = np.stack([orders_0, orders_1, orders_2])
    ...
    for i, name in enumerate(names):
        for k, N in enumerate(cell_counts):
            rows.append({
                "Scheme": name,
                "Mesh": int(N),
                r"$\rho$": f"{errors[0,i,k]:.2e}",
                "Order rho": "-" if np.isnan(orders[0,i,k]) else f"{orders[0,i,k]:.2f}",
                r"$u$": f"{errors[1,i,k]:.2e}",
                "Order u": "-" if np.isnan(orders[1,i,k]) else f"{orders[1,i,k]:.2f}",
                r"$p$": f"{errors[2,i,k]:.2e}",
                "Order p": "-" if np.isnan(orders[2,i,k]) else f"{orders[2,i,k]:.2f}"
            })
