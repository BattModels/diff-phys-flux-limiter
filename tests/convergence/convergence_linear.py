...
for n, n_cells in enumerate(cell_counts):
    ...
    x0, u0 = utils.square_wave(N=int(n_cells))

    for i, (name, flux_limiter) in enumerate(flux_limiters.items()):
        u, _ = fvm_solver.solve_linear_advection_1D(u0=u0, T=T, a=1., dx=x0[1]-x0[0], CFL=0.4, flux_limiter=flux_limiter)
        # errors[i, n] = np.sqrt(np.mean((u - u0)**2))
        errors[i, n] = np.mean(np.abs(u - u0))
        # fig, ax = plt.subplots()
